-- Adds the recipient's building unit / suite to the jsonb object returned by
-- get_tracking_details, alongside the street address it was missing from.
-- customer_unit has existed on the customer table for a while; the public
-- tracking page's frontend already reads recipient.unit defensively, so
-- this alone is what makes it show up there. Every other clause is
-- unchanged from the function's current definition.
