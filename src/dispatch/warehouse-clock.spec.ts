...
describe('localShiftDate', () => {
    it('is the UTC date at UTC', () => {
        expect(localShiftDate(new Date('2026-09-01T13:00:00Z'), 'UTC')).toBe(
            '2026-09-01',
        );
    });
    ...
    it('handles a half-hour offset', () => {
        expect(
            localShiftDate(new Date('2026-09-01T18:45:00Z'), 'Asia/Kolkata'),
        ).toBe('2026-09-02');
    });
    ...
    it('falls back to UTC for a zone Postgres knows and Node does not', () => {
        ...
        expect(
            localShiftDate(new Date('2026-09-01T13:00:00Z'), 'Mars/Olympus'),
        ).toBe('2026-09-01');
    });

    it('falls back to UTC when the column is null', () => {
        expect(localShiftDate(new Date('2026-09-01T13:00:00Z'), null)).toBe(
            '2026-09-01',
        );
    });
});
...
describe('utcOffsetSuffix', () => {
    it('reports a bare GMT as +00:00', () => {
        expect(utcOffsetSuffix(new Date('2026-09-01T00:00:00Z'), 'UTC')).toBe(
            '+00:00',
        );
    });
    ...
    it('follows daylight saving rather than assuming a fixed offset', () => {
        const winter = utcOffsetSuffix(
            new Date('2026-07-01T00:00:00Z'),
            'Australia/Melbourne',
        );
        const summer = utcOffsetSuffix(
...
describe('endOfLocalDayMs', () => {
    ...
    it('is always after the instant it was asked about', () => {
        ...
        expect(endOfLocalDayMs(at, 'Australia/Melbourne')).toBeGreaterThan(
            at.getTime(),
        );
    });
});
